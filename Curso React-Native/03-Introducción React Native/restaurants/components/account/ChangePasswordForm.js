...
export default function ChangePasswordForm({ setShowModal, toastRef }) {
    ...
    const validateForm = () => {
        ...
        if (isEmpty(currentPassword)) {
            setErrorCurrentPassword("Debe ingresar tu contraseña actual.");
            isValid = false;
        }

        if (size(newPassword) < 6) {
            setErrorNewPassword("Debe ingresar una nueva contraseña de al menos 6 carácteres.")
            isValid = false;
        }

        if (size(confirmPassword) < 6) {
            setErrorConfirmPassword("debe ingresar una nueva confirmación de tu contraseña de al menos 6 carácteres.")
            isValid = false;
        }
        ...
        if (newPassword === currentPassword) {
            setErrorCurrentPassword("Debes ingresar una contraseña diferente a actual.")
            setErrorNewPassword("Debes ingresar una contraseña diferente a actual.")
            setErrorConfirmPassword("Debes ingresar una contraseña diferente a actual.")
            isValid = false;
        }
